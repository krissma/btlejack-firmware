...
static void sniff_adv(uint8_t adv_channel, uint8_t mode)
{
  ...
  {
    /* Configure radio to receive packets on advertisement channels. */
    radio_follow_conn_le(g_sniffer.adv_pattern, adv_channel, 0x555555);
  }
  else if (mode == 0x02)
  {
    /* Configure radio to receive packets on advertisement channels. */
    radio_classic_follow_conn(g_sniffer.adv_pattern, adv_channel, 0x555555);
  }
  else
  {
    /* Configure radio to receive packets on advertisement channels. */
...
void dispatchMessage(T_OPERATION op, uint8_t *payload, int nSize, uint8_t ubflags)
{
  ...
  case ADVERTISEMENTS:
  {
    if ((ubflags & PKT_COMMAND) && (nSize >= 1))
    {
      ...
      {
        uint8_t status[] = {0x00};
        if (nSize != 3 && mode != 0x01 && mode != 0x02) /* Error, size does not match. */
        {
          status[0] = 0x01;
  ...
  case SEND_TEST_PKT:
  {
    if ((nSize >= 1) && (ubflags & PKT_COMMAND))
    {
      ...
      /* Extract pattern size. */
      uint8_t pattern_size = payload[2];

      /* Extract pattern. */
      for (int i = 0; i < pattern_size; i++)
      {
        pattern[i] = payload[3 + i];
      }

      if (payload[0] == 0x01)
      {
        radio_send_test_rx(pattern, pattern_size, payload, nSize, payload[1]);
        /* Send ACK. */
      ...
      {
        radio_classic_send_test_rx(pattern, pattern_size, payload, nSize, payload[1]);
        /* Send ACK. */
        pLink->sendPacket(SEND_PKT, 0, nSize, PKT_RESPONSE);
        //pLink->verbose(B("ftest"));
      }
